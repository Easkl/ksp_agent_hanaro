...
class PerformanceComparisonFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.method_cmp_button = ttk.Button(self, text="Comparison Chart", command=self.show_method_comparison)
        self.method_cmp_button.pack(pady=5)
    ...
    def show_method_comparison(self):
        ...
        # PSO ve DE sonuçları en sondalar:
        if len(self.performance_results) < 2:
            messagebox.showwarning("Warning",
                "Please run both PSO and DE at least once before comparison.")
            return
        ts_pso = self.performance_results[-2]['final_settling_time']
        os_pso = self.performance_results[-2]['overshoot']
        ts_de  = self.performance_results[-1]['final_settling_time']
        os_de  = self.performance_results[-1]['overshoot']

        # Z–N parametrelerini 텍스트로 표시
        try:
            Ku, Tu = compute_zn_params(app.final_plant)
            Kp_zn = 0.6 * Ku
            Ti    = 0.5 * Tu
            Td    = 0.125 * Tu
            Ki_zn = Kp_zn / Ti
            Kd_zn = Kp_zn * Td
            ts_zn, _, _, _, _, os_zn = ph.simulate_response(
                app.final_plant,
                {'Kp':Kp_zn, 'Ki':Ki_zn, 'Kd':Kd_zn},
                simulation_time=max(5, 4*app.target_settling_time)
            )
            comparison_text = (
                "Performance Comparison (Text-based):\n"
                f"Ziegler–Nichols: Settling Time = {ts_zn:.3f} s, Overshoot = {os_zn:.2f}%\n"
                f"PSO: Settling Time = {ts_pso:.3f} s, Overshoot = {os_pso:.2f}%\n"
                f"DE: Settling Time = {ts_de:.3f} s, Overshoot = {os_de:.2f}%\n"
            )
        except Exception as e:
            comparison_text = (
                "Performance Comparison (Text-based):\n"
                f"PSO: Settling Time = {ts_pso:.3f} s, Overshoot = {os_pso:.2f}%\n"
                f"DE: Settling Time = {ts_de:.3f} s, Overshoot = {os_de:.2f}%\n"
                f"Error in ZN calculation: {e}\n"
            )

        win = tk.Toplevel(self)
        win.title("Method Comparison")
        text_widget = scrolledtext.ScrolledText(win, width=80, height=10, font=("Helvetica", 10))
    ...
    def compare_runs(self):
        ...
        if not selected_indices:
            ...
            return
        comparison_text = "Performance Comparison (Text-based):\n"
        for idx in selected_indices:
            res = self.performance_results[idx]
            comparison_text += (
                f"Run {idx+1}: PID={res['pid_params']}, "
                f"Settling Time={res['final_settling_time']:.3f}s, "
                f"Overshoot={res['overshoot']:.2f}%\n"
            )
        comp_window = tk.Toplevel(self)
        comp_window.title("Performance Comparison")
        text_widget = scrolledtext.ScrolledText(comp_window, width=80, height=15, font=("Helvetica", 10))
        text_widget.insert(tk.END, comparison_text)
        text_widget.configure(state="disabled")
        text_widget.pack(fill="both", expand=True)
...
class InteractiveDesignWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        for alg_name in ALGORITHM_REGISTRY:
        ...
        self.rt_text_label = ttk.Label(rt_frame, text="Real-time metrics will be displayed here.", font=("Helvetica", 10))
        self.rt_text_label.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def update_realtime_simulation(self, event):
        ...
        self.rt_text_label.config(text=metrics_text)
    ...
    def run_optimization(self):
        ...
        alg = ALGORITHM_REGISTRY[self.optimization_method.get()]
        optimized_pid = alg(self.plant,
                    self.initial_target_settling_time,
                    self.target_overshoot,
                    self.target_rise_time,
                    self.pid_params)
...
class ProjectManagerFrame(ttk.Frame):
    ...
    def save_project(self):
        ...
        # 그래프 없이 텍스트 요약 저장
        project_data["simulation_summary"] = (
            f"Settling Time: {settling_time:.3f} s\n"
            f"Rise Time: {rise_time:.3f} s\n"
            f"Overshoot: {overshoot:.2f}%\n"
        )
    ...
    def load_project(self):
        ...
        if file_path:
            ...
            summary = f"PID: {self.app.final_pid_params}\n" \
                      ...
                      f"Simulation Summary: {project_data.get('simulation_summary', 'N/A')}"
...
def open_faq(master):
    ...
    faq_text = (
        ...
        "Tip: Avoid overly long or deeply nested conditions. "
        "If the values extracted by the LLM appear inconsistent, you can correct them using the manual input table.\n\n"
        "**Explanation of Performance Metrics:**\n\n"
        "**Settling Time:**\n"
        "This refers to the time it takes for the system output to stabilize within a certain range around the target value, after initial fluctuations.\n"
        "- Example: “...settling time shorter than 2.2 seconds” ⇒ A settling time shorter than 2.2 seconds means the system should reach and stay near the desired value quickly.\n\n"
        "**Overshoot:**\n"
        "Overshoot is how much the system output exceeds the target value before settling.\n"
        "- Example: “...a maximum overshoot of 5%” ⇒ A maximum overshoot of 5% means the system should not go more than 5% above the desired value, which helps prevent instability or damage.\n\n"
        "**Rise Time:**\n"
        "Rise time is how fast the system output goes from its initial value to the target range (usually from 10% to 90% of the final value).\n"
        "- Example: “...a rise time faster than 0.5 seconds” ⇒ A rise time faster than 0.5 seconds means the system is expected to respond quickly to changes.\n\n"
        "Note: These performance metrics are widely used in control system design. While shorter times often suggest better responsiveness, they must be chosen carefully to ensure the system remains stable and safe.\n"
    )
