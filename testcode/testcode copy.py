...
class PerformanceComparisonFrame(ttk.Frame):
    ...
    def show_method_comparison(self):
        ...
        # Z–N parametrelerini manuel frekans taraması ile al
        try:
            ...
            Ki_zn = Kp_zn / Ti
            Kd_zn = Kp_zn * Td
            # simülasyonla performans
            ts_zn, _, _, _, _, os_zn = ph.simulate_response(
                app.final_plant,
                {'Kp':Kp_zn, 'Ki':Ki_zn, 'Kd':Kd_zn},
                simulation_time=max(5, 4*app.target_settling_time)
            )
            methods   = ["Ziegler–Nichols", "PSO", "DE"]
            settling  = [ts_zn, ts_pso, ts_de]
            overshoot = [os_zn, os_pso, os_de]
        except Exception as e:
            # Her şeye rağmen bir hata çıkarsa yalnizca PSO/DE
            methods   = ["PSO", "DE"]
            settling  = [ts_pso, ts_de]
            overshoot = [os_pso, os_de]

        # Grafiği çiz
        x     = np.arange(len(methods))
        width = 0.35
        fig, ax = plt.subplots(figsize=(6,4))
        ax.bar(x - width/2, settling,  width, label="Settling Time (s)")
        ax.bar(x + width/2, overshoot, width, label="Overshoot (%)")
        ax.set_xticks(x)
    ...
    def compare_runs(self):
        ...
        toolbar.update()
        canvas.draw()
        comp_window.protocol("WM_DELETE_WINDOW", comp_window.destroy)
